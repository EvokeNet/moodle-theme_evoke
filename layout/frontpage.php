...
$themesettings = new \theme_evoke\util\theme_settings();
...
if (isloggedin()) {
    ...
    $alertmsg = theme_evoke_get_setting('alertmsg');
    ...
    theme_evoke_extend_flat_navigation($PAGE->flatnav);
    ...
    if ((theme_evoke_get_setting('sliderenabled', true) == true) && (theme_evoke_get_setting('sliderfrontpage', true) == true)) {
        $sliderfrontpage = true;
        $extraclasses[] = 'slideshow';
    }

    $numbersfrontpage = false;
    if (theme_evoke_get_setting('numbersfrontpage', true) == true) {
        $numbersfrontpage = true;
    }

    $sponsorsfrontpage = false;
    if (theme_evoke_get_setting('sponsorsfrontpage', true) == true) {
        $sponsorsfrontpage = true;
    }

    $clientsfrontpage = false;
    if (theme_evoke_get_setting('clientsfrontpage', true) == true) {
        $clientsfrontpage = true;
    }

    $bannerheading = '';
    if (!empty($PAGE->theme->settings->bannerheading)) {
        $bannerheading = theme_evoke_get_setting('bannerheading', true);
    }

    $bannercontent = '';
    if (!empty($PAGE->theme->settings->bannercontent)) {
        $bannercontent = theme_evoke_get_setting('bannercontent', true);
    }

    $shoulddisplaymarketing = false;
    if (theme_evoke_get_setting('displaymarketingbox', true) == true) {
        $shoulddisplaymarketing = true;
    }

    $disablefrontpageloginbox = false;
    if (theme_evoke_get_setting('disablefrontpageloginbox', true) == true) {
    ...
    echo $OUTPUT->render_from_template('theme_evoke/frontpage_guest', $templatecontext);
}
